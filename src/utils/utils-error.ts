...
export const getErrorMessage = (error: any) => {
  if (isAxiosError(error)) {
    const message =
      ...
      Object.values(get(error, 'response.data.errors') || [])
        .map(item => {
          const attr = get(item, 'attr');
          return [
            attr !== 'non_field_errors' && upperFirst(attr),
            get(item, 'detail'),
          ]
            .filter(Boolean)
            .join(' - ');
    ...
    return {
      ...
      type: get(error, 'response.data.type'),
  ...
  return {
    ...
    statusText: get(error, 'statusText') || 'Internal Server Error',
    message: message || 'Something went wrong. Please try again later.',
  };
};
...
export const generateFirebaseAuthErrorMessage = (errorCode: any) => {
  switch (errorCode) {
